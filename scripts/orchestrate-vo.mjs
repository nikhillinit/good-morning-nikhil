...
const draftVoTask = defineTask('vo-drafter', (args) => ({
  ...
  prompt: [
    ...
    `Apply SK-MAC-007 Vocal Direction Standards for character consistency:`,
    ``,
    `### Vocal Direction (Steve)`,
    `- **Primary Quality**: Raspy, powerful`,
    `- **Texture**: Worn, gritty`,
    `- **Register**: Baritone (A2-A4, Warm/Versatile)`,
    `- **Techniques**: Signature exasperated sighs, deliberate pauses`,
    `- **Emotional Delivery**: Anguished, Defiant (snappy, exhausted, irritated)`,
    `- **Influences**: Steve Harvey (incredulous delivery, tired authority)`,
    ``,
    `### Vocal Direction (Jeff)`,
    `- **Primary Quality**: Clear, ethereal`,
    `- **Texture**: Pure, overly polished`,
    `- **Register**: Tenor`,
    `- **Techniques**: Deliberate tempo shifts, slight whisper for intimacy`,
    `- **Emotional Delivery**: Confident, Intimate (smug and beautifully unbothered)`,
    `- **Influences**: Jeff Goldblum (eccentric, halting, smug rhythm)`,
    ``,
    args.feedback
      ? `Refine based on Director's critique: ${args.feedback}\nPrevious Draft: ${args.previousScript}`
      : `Initial prompt/context: ${args.prompt}`,
    ``,
    `Return ONLY a JSON object with a single key "script" whose value is the full dialogue with stage directions matching these SK-MAC-007 presets.`,
  ].join('\n'),
}));
...
const scoreVoTask = defineTask('vo-scorer', (args) => ({
  ...
  prompt: [
    ...
    `Score the script out of 100 based on standard broadcast pacing and adherence to the SK-MAC-007 Vocal Direction matrix.`,
    `Validation rules:`,
    `- Does Steve hit the "Raspy, powerful" Baritone descriptors? Exasperated sighs present?`,
    `- Does Jeff utilize "Clear, ethereal" Tenor characteristics? Intimate whispering or halting rhythms?`,
    `- Emotional delivery contrast: Anguished/Defiant vs Confident/Intimate.`,
    ``,
    `Script to evaluate:`,
    `${args.script}`,
    ``,
